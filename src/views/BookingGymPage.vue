    ...
    <v-card>
      ...
      <v-data-table
          ...
          <td :colspan="headers.length">
            <v-btn v-if="item.status_presensi == 0 && isToday(item.tanggal_booking)" text color="purple" @click="submit(item)"> Take Attendance</v-btn>
            <v-btn v-else-if="item.status_presensi == 0 && isPastDate(item.tanggal_booking)" text color="purple" @click="submit(item)" disabled> Take Attendance</v-btn>
...
methods: {
  ...
  getDataBooking(){
      this.load = true;
      var url = this.$api + "/bookingGymPegawai";
      this.$http
      .get(url, {
          headers: {
